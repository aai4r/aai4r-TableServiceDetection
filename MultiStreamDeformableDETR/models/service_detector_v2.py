...
# v2: add give_infomration only limited options
...
class ImageEncoderROIAttBasedSAClassifier(ServiceAlarmClassifier):
    ...
    def __init__(self, num_classes, num_trfm,
                 ...
                 apply_mask_by_class=False, apply_mask_by_bbox=False,
                 apply_mask_by_prog_duration=False):
        ...
        assert use_duration
        assert limit_det_class
        self.use_merge_det_amount = use_merge_det_amount
        assert apply_nms_on_pca
        self.apply_btl_backbone = apply_btl_backbone
        self.apply_btl_roi = apply_btl_roi
        assert apply_one_value_amount

        self.apply_mask_by_class = apply_mask_by_class
        self.apply_mask_by_bbox = apply_mask_by_bbox
        self.apply_mask_by_prog_duration = apply_mask_by_prog_duration
        ...
        assert self.pca_aggregate_type == 'attn_simple'
        ...
        if self.use_pca:
            ...
            if self.pca_aggregate_type == 'attn_qkv':
                ...
                if self.use_merge_det_amount:
                    self.attn_pca_detclass_amount = nn.ModuleList([nn.Linear(self.pca_amount_dim+self.pca_detclass_dim, 1) for _ in range(self.num_classes)])
                else:
                    self.attn_pca_amount = nn.ModuleList([nn.Linear(self.pca_amount_dim, 1) for _ in range(self.num_classes)])
                    self.attn_pca_detclass = nn.ModuleList([nn.Linear(self.pca_detclass_dim, 1) for _ in range(self.num_classes)])
    ...
    def forward(self, x):
        ...
        if self.use_pca:
            ...
            if self.pca_aggregate_type == 'attn_simple':
                mask_by_class, mask_by_bbox, \
                mask_by_prog_duration = get_classwise_weight(x_pred_logits, x['pred_boxes'],
                                                             self.dict_used_det_classname)

                # same shape and only reduce 0 on weight is the best
                if self.use_merge_det_amount:
                    x_pred_logits[~nms_index_masks] = self.no_value_for_pred        # x_pred_logits has 0. ~ 1.
                    ...
                    list_pred_amount_agg = []
                    list_x_det_amount_weight_swap = []
                    for i_c in range(self.num_classes):
                        masked_x_pred_logits = x_pred_logits
                        masked_x_amount_score_nz = x_amount_score_nz

                        if self.apply_mask_by_class:
                            masked_x_pred_logits[:, :, ~mask_by_class[i_c]] = self.no_value_for_pred

                        if self.apply_mask_by_bbox:
                            masked_x_pred_logits[~mask_by_bbox[i_c]] = self.no_value_for_pred
                            masked_x_amount_score_nz[~mask_by_bbox[i_c]] = self.no_value_for_amount

                        masked_x_det_amount = torch.cat([masked_x_pred_logits, masked_x_amount_score_nz], dim=2)  # 16, 1200, 11
                        x_det_amount_weight_score = self.attn_pca_detclass_amount[i_c](masked_x_det_amount)  # 11 > 1, [", ", n_det_class+amount] -> [n_batch, n_roi, 1]

                        if self.apply_mask_by_bbox:
                            x_det_amount_weight = masked_softmax(x_det_amount_weight_score, mask_by_bbox[i_c], dim=1)  # [n_batch, n_roi, 1]
                        else:
                            x_det_amount_weight = torch.softmax(x_det_amount_weight_score, dim=1)  # [n_batch, n_roi, 1]
                        x_det_amount_weight_swap_pc = x_det_amount_weight.permute(0, 2, 1)  # [n_b, 1, n_roi]

                        pred_amount_agg_pc = torch.matmul(x_det_amount_weight_swap_pc, masked_x_det_amount)  # > [n_b, 1, 21]
                        pred_amount_agg_pc = pred_amount_agg_pc.permute(1, 0, 2)  # [1, n_b, 21]

                        list_pred_amount_agg.append(pred_amount_agg_pc)
                        list_x_det_amount_weight_swap.append(x_det_amount_weight_swap_pc)

                    pdb.set_trace()
                    pred_amount_agg = torch.cat(list_pred_amount_agg, dim=0)
                    x_det_amount_weight_swap = torch.cat(list_x_det_amount_weight_swap, dim=0)
                else:
                    # [16, 5, sum_to_1]
                    x_pred_logits[~nms_index_masks] = self.no_value_for_pred
                    nms_nz_index_masks = nms_nz_index_masks[:, :, self.amount_index].sum(dim=2).type(torch.bool)  # [n_b, 900, n_det_class(10)] > [n_b, 900]
                    x_amount_score_nz[~nms_nz_index_masks] = self.no_value_for_amount  # v3, 0. -> 1.

                    list_pred_logits_agg = []
                    list_x_pred_logits_weight_swap = []
                    list_amount_score_agg = []
                    list_x_amount_score_weight_swap = []
                    for i_c in range(self.num_classes):
                        # x_pred_logits
                        masked_x_pred_logits = x_pred_logits.clone()    # [n_b, n_roi, n_det_classes]
                        ...
                        x_pred_logits_weight_score = self.attn_pca_detclass[i_c](masked_x_pred_logits)  # [", ", n_det_class] -> [n_batch, n_roi, 1]

                        if self.apply_mask_by_bbox:
                            x_pred_logits_weight = masked_softmax(x_pred_logits_weight_score, mask_by_bbox[i_c], dim=1)  # [n_batch, n_roi, 1]
                        else:
                            x_pred_logits_weight = torch.softmax(x_pred_logits_weight_score, dim=1)  # [n_batch, n_roi, 1]
                        x_pred_logits_weight_swap_pc = x_pred_logits_weight.permute(0, 2, 1)   # [n_b, 1, n_roi]

                        pred_logits_agg_pc = torch.matmul(x_pred_logits_weight_swap_pc, masked_x_pred_logits)  # > [n_b, n_class(1), 21]
                        pred_logits_agg_pc = pred_logits_agg_pc.permute(1, 0, 2)    # [n_class(1), n_b, 21]

                        list_pred_logits_agg.append(pred_logits_agg_pc)
                        list_x_pred_logits_weight_swap.append(x_pred_logits_weight_swap_pc)

                        # x_amount_score
                        masked_x_amount_score_nz = x_amount_score_nz.clone()

                        if self.apply_mask_by_bbox:
                            # masked_x_pred_logits[~mask_by_bbox[i_c]] = self.no_value_for_pred
                            _, ms2, ms3 = mask_by_bbox.shape
                            nz_mask_by_bbox_ic = mask_by_bbox[i_c][nz_index].view(ms2, -1)
                            masked_x_amount_score_nz[~nz_mask_by_bbox_ic] = self.no_value_for_amount # [16, 900, 1], [16, 1200]

                        x_amount_score_weight_score = self.attn_pca_amount[i_c](masked_x_amount_score_nz)  # [n_b, nz_roi, 50] > [", ", n_class(5)]

                        if self.apply_mask_by_bbox:
                            x_amount_score_weight = masked_softmax(x_amount_score_weight_score, nz_mask_by_bbox_ic, dim=1)
                        else:
                            x_amount_score_weight = torch.softmax(x_amount_score_weight_score, dim=1)
                        x_amount_score_weight_swap_pc = x_amount_score_weight.permute(0, 2, 1) #

                        amount_score_agg_pc = torch.matmul(x_amount_score_weight_swap_pc, masked_x_amount_score_nz)
                        amount_score_agg_pc = amount_score_agg_pc.permute(1, 0, 2)

                        list_amount_score_agg.append(amount_score_agg_pc)
                        list_x_amount_score_weight_swap.append(x_amount_score_weight_swap_pc)

                    pred_logits_agg = torch.cat(list_pred_logits_agg, dim=0)        # list of [1, 16, 10]
                    x_pred_logits_weight_swap = torch.cat(list_x_pred_logits_weight_swap, dim=1)    # list of [16, 1, 1200]
                    amount_score_agg = torch.cat(list_amount_score_agg, dim=0)      # list of [1, 16, 1]
                    x_amount_score_weight_swap = torch.cat(list_x_amount_score_weight_swap, dim=1)  # list of [16, 1, 900]
                ...
                if self.apply_mask_by_prog_duration:
                    x_progress_score[~mask_by_prog_duration.squeeze()] = self.no_value_for_progress
                    x_duration[~mask_by_prog_duration.squeeze()] = self.no_value_for_duration
...
def build_SAclassifier(num_classes, num_trfm, saclassifier_type='imagebased'):
    ...
    elif saclassifier_type == 'imageavgp_pca1dlcnmsmcattnsimple':
        model = ImageEncoderROIAttBasedSAClassifier(num_classes, num_trfm, use_backbone=True,
                                                    backbone_aggregate_type='avgpool',
                                                    use_encoder=False, use_roi=False,
                                                    use_pca=True, use_duration=True,
                                                    limit_det_class=True, apply_nms_on_pca=True,
                                                    apply_one_value_amount=True,
                                                    apply_mask_by_class=True,
                                                    aggregate_type='attn_simple')
        # postprocessors['bbox_attn'] = PostProcess()

    elif saclassifier_type == 'imageavgp_pca1dlcnmsmbattnsimple':
        model = ImageEncoderROIAttBasedSAClassifier(num_classes, num_trfm, use_backbone=True,
                                                    backbone_aggregate_type='avgpool',
                                                    use_encoder=False, use_roi=False,
                                                    use_pca=True, use_duration=True,
                                                    limit_det_class=True, apply_nms_on_pca=True,
                                                    apply_one_value_amount=True,
                                                    apply_mask_by_bbox=True,
                                                    aggregate_type='attn_simple')
        # postprocessors['bbox_attn'] = PostProcess()

    elif saclassifier_type == 'imageavgp_pca1dlcnmsmdattnsimple':
        model = ImageEncoderROIAttBasedSAClassifier(num_classes, num_trfm, use_backbone=True,
                                                    backbone_aggregate_type='avgpool',
                                                    use_encoder=False, use_roi=False,
                                                    use_pca=True, use_duration=True,
                                                    limit_det_class=True, apply_nms_on_pca=True,
                                                    apply_one_value_amount=True,
                                                    apply_mask_by_prog_duration=True,
                                                    aggregate_type='attn_simple')
        # postprocessors['bbox_attn'] = PostProcess()
