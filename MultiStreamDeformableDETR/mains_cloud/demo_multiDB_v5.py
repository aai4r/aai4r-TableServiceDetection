...
def main(args, imgs_dir=None, output_dir=None):
    ...
    pps = postprocessor(target_class_list=args.display_class_names, class_names=classes)
    ...
    with torch.no_grad():
        ...
        for i_th, img_file in enumerate(list_files[args.skip_first_n_image:]):
            ...
            if cur_cap_sec - last_cap_sec > duration_sec:
                ...
                amount_pred = torch.cat(filtered_amount_pred, dim=0)  # [n_bboxes, 3]
                amount_pred_weighted = torch.cat(filtered_amount_pred_weighted, dim=0)  # [n_bboxes]

                # # each result to detections
                # detections = torch.cat([labels.unsqueeze(1).float(), scores.unsqueeze(1),
                #                         boxes,
                #                         amount_pred, amount_pred_weighted.unsqueeze(1)], dim=1)
                det_trkform = det_to_trk(labels, scores, boxes, amount_pred, amount_pred_weighted,
                                         index_to_classname=pps.class_names,
                                         cap_time=[im_hrs, im_min, im_sec])
                im_np = np.asarray(im.convert('L'))
                tracks = pps.process(detections=det_trkform, frame=im_np)
                tracks_dup = [{key: value for key, value in item.items() if key != 'visual_tracker'}
                              for item in tracks]
                # alarms = pps.check_alarm(tracks=tracks_dup, progress_prob=progress_prob,
                #                          cap_time=[im_hrs, im_min, im_sec])
                pps.remove_over_trk(cap_time=[im_hrs, im_min, im_sec],
                                    th_over_duration_in_sec=pps.alarm_duration_sec)

                # # save detection, tracking and alarming results
                # if args.save_results:
                #     save_dict = {'labels': labels, 'scores': scores, 'boxes': boxes,
                #                 'amount_pred': amount_pred,
                #                 'amount_pred_weighted': amount_pred_weighted,
                #                 'tracks': tracks_dup,
                #                 'alarms': alarms}
                #     try:
                #         with open(filename_det, 'wb') as f:
                #             pickle.dump(save_dict, f)
                #     except:
                #         pdb.set_trace()

                # labels = detections[:, 0].int()
                # scores = detections[:, 1]
                # boxes = detections[:, 2:6]
                # amount_pred = detections[:, 6:9]
                # amount_pred_weighted = detections[:, 9]

                # plot_results
                # draw information from modelG
                #   : bbox, class, its amount
                #   : progress
                # draw info from modelH
                #   : bbox, attention
                #   : sa-class
                ...
                # for ith, (label, score, (xmin, ymin, xmax, ymax)) in enumerate(
                #         zip(labels.tolist(), scores.tolist(), boxes.tolist())):
                for ith, item in enumerate(tracks):
                    try:
                        class_name = item['class']
                    except:
                        class_name = item['rep_class']
                    ...
                    if class_name in ['food', 'drink', 'dish', 'cup']:
                        amount_pred = item['amount_final']
                        amount_pred_weighted = item['amount_w_final']
                    else:
                        amount_pred = amount_pred_weighted = None

                    if True:  # draw history
                        for i_box in range(len(item['bboxes']) - 1):
                            cx1 = sum(item['bboxes'][i_box][0::2]) / 2.
                            cy1 = sum(item['bboxes'][i_box][1::2]) / 2.

                            cx2 = sum(item['bboxes'][i_box + 1][0::2]) / 2.
                            cy2 = sum(item['bboxes'][i_box + 1][1::2]) / 2.

                            draw.line((cx1, cy1, cx2, cy2), fill='white', width=1)

                    if args.use_amount and amount_pred is not None:
                ...
                # draw areas
                for item in pps.points:
                    draw.point((item[0], item[1]), fill='white')
